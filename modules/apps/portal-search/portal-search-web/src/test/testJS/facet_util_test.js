...
describe('Liferay.Search.FacetUtil', () => {
	before(done => {
		AUI().use('liferay-search-facet-util', A => {
			done();
		});
	});

	describe('.removeURLParameters()', () => {
		it('should remove the parameter whose name is the given key.', done => {
		...
		it('should NOT remove parameters not matching the given key.', done => {
			var parameterArray = [
	...
	describe('unit', () => {
		describe('.setURLParameter()', () => {
			it('should add a missing parameter.', done => {
			...
			it('should update an existing parameter.', done => {
				var url = Liferay.Search.FacetUtil.setURLParameter(
			...
			it('should add a missing parameter with path.', done => {
				var url = Liferay.Search.FacetUtil.setURLParameter(
		...
		describe('.setURLParameters()', () => {
			it('should add new selections.', done => {
			...
			it('should remove old selections.', done => {
				var parameterArray = Liferay.Search.FacetUtil.setURLParameters(
					'key',
					['sel2', 'sel3'],
					['key=sel1']
			...
			it('should preserve other selections.', done => {
				var parameterArray = Liferay.Search.FacetUtil.setURLParameters(
					'key1',
					['sel1'],
					['key2=sel2']
		...
		describe('.removeURLParameters()', () => {
			it('remove given parameter.', done => {
			...
			it('should preserve other parameters.', done => {
				var parameterArray = Liferay.Search.FacetUtil.removeURLParameters(
					'key1',
					['key1=sel1', 'key2=sel2']
				);
		...
		describe('.updateQueryString()', () => {
			...
			it('should accept query string without question mark.', done => {
				var queryString = Liferay.Search.FacetUtil.updateQueryString(
					'key1',
					['sel1'],
					'key2=sel2'
	...
	describe('regression', () => {
		describe('.updateQueryString()', () => {
			it('should not prefix with ampersand.', done => {
				var queryString = Liferay.Search.FacetUtil.updateQueryString(
					'key',
					['sel1', 'sel2'],
					'?'
