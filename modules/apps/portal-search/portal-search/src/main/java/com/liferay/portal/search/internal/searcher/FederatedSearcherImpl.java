...
public class FederatedSearcherImpl implements FederatedSearcher {

	@Override
	public Hits getHits(SearchContext searchContext) {
		SearchSearchRequest searchSearchRequest = new SearchSearchRequest();

		searchSearchRequest.setIndexNames(new String[]{"federated_youtube"});
		...
		Query federatedTitleQuery = new MatchQuery(
			"regularTextTitle", keywords);

		Query federatedContentQuery = new MatchQuery(
			"regularTextContent", keywords);
		...
		//only needed if federated_youtube index is not in same cluster as liferay index
	...
	public String getSourceDisplayName() {
		//must match FederatedSearchResultSummaryBuilderImpl.getSourceDisplayName()
		return "Federated One";
	}
