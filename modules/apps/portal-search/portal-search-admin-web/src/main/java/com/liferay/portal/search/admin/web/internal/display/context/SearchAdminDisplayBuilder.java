...
public class SearchAdminDisplayBuilder {
	...
	public SearchAdminDisplayContext build() {
		...
		addNavigationItemList(
			navigationItemList, request, "index-actions", tab);

		if (indexInformationAvailable) {
			addNavigationItemList(
				navigationItemList, request, "field-mappings", tab);
		}
	...
	protected void addNavigationItemList(
		NavigationItemList navigationItemList, HttpServletRequest request,
		String label, String selectedTab) {
		...
		navigationItemList.add(
			navigationItem -> {
				...
				navigationItem.setLabel(_language.get(request, label));
			});
	}
	...
	protected String getTab() {
		...
		if (isIndexInformationAvailable() &&
			Objects.equals(tab, "field-mappings")) {
