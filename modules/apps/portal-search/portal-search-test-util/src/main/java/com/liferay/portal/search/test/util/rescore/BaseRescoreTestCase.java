...
public abstract class BaseRescoreTestCase extends BaseIndexingTestCase {
	...
	public void testRescore() {
		...
		assertSearch(
			fieldName, query, (List<Rescore>)null,
			Arrays.asList("alpha alpha", "alpha zeta", "alpha beta beta"));

		assertSearch(
			fieldName, query, Arrays.asList(buildRescore(fieldName, "beta")),
	...
	public void testRescoreQuery() {
		...
		assertSearch(
			fieldName, query, (Query)null,
			Arrays.asList(
				"alpha alpha", "alpha gamma gamma", "alpha beta beta"));

		Query rescoreQuery = queries.match(fieldName, "beta");

		assertSearch(
			fieldName, query, rescoreQuery,
			Arrays.asList(
	...
	public void testRescores() {
		...
		assertSearch(
			fieldName, query,
			Arrays.asList(
				buildRescore(fieldName, "beta"),
				buildRescore(fieldName, "gamma")),
			Arrays.asList(
	...
	protected String getFieldValue(String fieldName, SearchHit searchHit) {
		...
		com.liferay.portal.search.document.Field field = fields.get(fieldName);
