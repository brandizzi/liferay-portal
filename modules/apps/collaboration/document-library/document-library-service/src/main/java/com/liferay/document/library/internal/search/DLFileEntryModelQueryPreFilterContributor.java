...
public class DLFileEntryModelQueryPreFilterContributor
	...
	public void contribute(
		...
		//added from DLFileEntryIndexer.postProcessContextBooleanFilter()
	...
	protected void addHelperFilters(BooleanFilter fullQueryBooleanFilter,
			SearchContext searchContext) {

		//added from DLFileEntryIndexer.postProcessContextBooleanFilter() - call to BaseIndexer method
		//move to QueryBuilderImpl? depends on WorkflowedModel
		modelQueryPreFilterContributorHelper.addStatusFilter(
			fullQueryBooleanFilter, searchContext);

		//added from DLFileEntryIndexer.postProcessContextBooleanFilter() - call to BaseIndexer method
		modelQueryPreFilterContributorHelper.addSearchClassTypeIdsFilter(
			fullQueryBooleanFilter, searchContext);

		//from _createPreFilterForEntryClassName
		//basically added always, added here because we have control over the class name
		//allows us to avoid implementing a SearchPermissionFilterContributor?
		modelQueryPreFilterContributorHelper.addPermissionFilter(
			fullQueryBooleanFilter, DLFileEntry.class.getName(), searchContext);

		//from _createPreFilterForEntryClassName
		//basically added always, added because setStagingAware was not set to false in DLFileEntryIndexer
		//move to QueryBuilderImpl? depends on StagedModel
