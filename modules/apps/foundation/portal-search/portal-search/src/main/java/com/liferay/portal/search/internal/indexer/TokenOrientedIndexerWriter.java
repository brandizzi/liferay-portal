 ...
 */
public class TokenOrientedIndexerWriter<T extends BaseModel<?>>
	implements IndexerWriter<T> {

	private IndexerTokenConsumer _indexerTokenConsumer;
	public TokenOrientedIndexerWriter(
		...
		IndexerTokenConsumer indexerTokenConsumer, boolean enabled) {
		...
		_indexerTokenConsumer = indexerTokenConsumer;
		_enabled = enabled;
	}
	...
	public void delete(long companyId, String uid) {
		...
		_indexerTokenConsumer.execute(createDeleteIndexerToken(companyId, uid));
	}
	...
	public void reindex(String[] ids) {
		...
		for (String id : ids) {
			...
			_indexerTokenConsumer.execute(indexerToken);
		}
	}
	...
	public void reindex(T baseModel) {
		...
		if ((indexerWriterMode == IndexerWriterMode.UPDATE) ||
			...
			_indexerTokenConsumer.execute(createReindexModelIndexerToken(baseModel));
		}
	...
	public void updatePermissionFields(T baseModel) {
		_indexerTokenConsumer.execute(createUpdatePermissionFieldsIndexerToken(baseModel));
	}
	...
	private static final Log _log = LogFactoryUtil.getLog(
		TokenOrientedIndexerWriter.class);
