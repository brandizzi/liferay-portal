...
public class UpgradeDDLRecordSetSettings extends UpgradeProcess {
	...
	protected boolean containsRequireAuthenticationSetting(
		...
		for (int i = 0; i < fieldValues.length(); i++) {
			JSONObject fieldValue = fieldValues.getJSONObject(0);
	...
	protected void doUpgrade() throws Exception {
		String sql = "select recordSetId, settings_ from DDLRecordSet";
		...
		try (PreparedStatement ps1 = connection.prepareStatement(sql);
			...
			while (rs.next()) {
				...
				if (Validator.isNotNull(settings)) {
					...
					if (!containsRequireAuthenticationSetting(fieldValues)) {
