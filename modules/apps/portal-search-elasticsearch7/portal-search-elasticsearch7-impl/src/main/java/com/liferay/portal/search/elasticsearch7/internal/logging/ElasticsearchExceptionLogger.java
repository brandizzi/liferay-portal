 ...
 */
public class ElasticsearchExceptionLogger {

	public ElasticsearchExceptionLogger(Log log, boolean logExceptionsOnly) {
		_log = log;
		_logExceptionsOnly = logExceptionsOnly;
	}

	public <T extends Throwable> void logOrThrow(T throwable) throws T {
	...
	public <T extends Throwable> void logOrThrowOnDeleteDocumentFailure(
			T throwable)
		throws T {

		if (isIndexNotFound(throwable)) {
			if (_log.isInfoEnabled()) {
				_log.info(throwable, throwable);
			}
		}
		else {
			logOrThrow(throwable);
		}
	}
