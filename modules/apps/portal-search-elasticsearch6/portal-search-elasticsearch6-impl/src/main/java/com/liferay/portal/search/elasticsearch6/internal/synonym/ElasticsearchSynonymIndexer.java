...
public class ElasticsearchSynonymIndexer implements SynonymIndexer {
	...
	@Override
	public String[] getSynonymSets(long companyId) {
		return getSynonymSets(indexNameBuilder.getIndexName(companyId));
	}

	@Override
	public String[] getSynonymSets(String indexName) {
		Settings settings = getSettings(indexName, _SYNONYN_SETS_PATH);

		List<String> synonyms = settings.getAsList(_SYNONYN_SETS_PATH);

		return synonyms.toArray(new String[synonyms.size()]);
	}

	@Override
	public void updateSynonymSets(long companyId, String[] synonymSets)
		throws SynonymException {

		updateSynonymSets(
			indexNameBuilder.getIndexName(companyId), synonymSets);
	}

	@Override
	public void updateSynonymSets(String indexName, String[] synonymSets)
		throws SynonymException {

		Settings settings = getSettings(synonymSets);
	...
	protected Settings getSettings(String indexName, String settingName) {
		IndicesAdminClient indicesAdminClient = getIndicesAdminClient();

		GetSettingsRequestBuilder getSettingsRequestBuilder =
			indicesAdminClient.prepareGetSettings(indexName);

		getSettingsRequestBuilder.setNames(settingName);
	...
	protected Settings getSettings(String[] synonymSets) {
		Settings.Builder builder = Settings.builder();

		builder.putList(_SYNONYN_SETS_PATH, synonymSets);
	...
	private static final String _SYNONYN_SETS_PATH =
		"index.analysis.filter.english_synonyms.synonyms";
