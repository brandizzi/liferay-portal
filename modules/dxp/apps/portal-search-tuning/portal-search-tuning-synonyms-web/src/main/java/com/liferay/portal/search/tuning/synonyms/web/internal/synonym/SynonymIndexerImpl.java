...
public class SynonymIndexerImpl implements SynonymIndexer {
	...
	public void updateSynonymSets(
		...
		_log.error("LPS-100272 " + clazz.getName() + ": closing index " + indexName);

		closeIndex(indexName);

		try {
			_log.error("LPS-100272 " + clazz.getName() + ": updating setings on " + indexName);
			...
			_log.error("LPS-100272 " + clazz.getName() + ": setings updated on" + indexName);
		}
		finally {
			openIndex(indexName);
			_log.error("LPS-100272 " + clazz.getName() + ": index reopened > " + indexName);
		}
	}
