...
package com.liferay.portal.search.tuning.synonyms.web.internal.activator;
 ...
 */
@Component(service = SynonymSetIndexCreationBundleActivator.class)
public class SynonymSetIndexCreationBundleActivator {
	...
	protected void activate() {
		...
		stream.map(
			...
			indexName -> !_synonymSetIndexHelper.isExists(indexName)
		).forEach(
			this::createSynonymSetIndex
		);
	}

	protected void createSynonymSetIndex(String indexName) {
		_synonymSetIndexHelper.create(indexName);

		_synonymSetFilterIndexSynchronizer.copyFiltersToIndex(indexName);
	}
	...
	@Reference
	private SynonymSetIndexCreator _synonymSetIndexHelper;
